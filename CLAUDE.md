...
## Budget module ("ניהול תקציב") - Stages 1-6 CLOSED / PASS (Stages 3-6 LOCAL only)

Stages 3 (Budget Core), 4 (Documents & Supplier Workflow), 5 (Party Funding Workflow) and 6 (Dashboard, Reports & Controls) are local commits only - NOT pushed, NOT deployed, their eight migrations (`20260917000000`/`010000`/`020000`, `20260918000000`/`010000`, `20260919000000`/`010000`, `20260920000000`) NOT applied to Production, Budget not activated anywhere. Stage 7 (Integration, Security & Production Acceptance) is NOT STARTED and needs explicit approval. Approved design: `CURRENT_STATUS.md`'s Budget section; the Stage 3-6 build records follow it - do not re-derive or reopen them.
...
- **Tests:** `scripts/budget/api-budget.mjs`, `ui-budget.mjs`, `api-voterfile.mjs`, `api-budget-docs.mjs`, `ui-budget-docs.mjs`, `api-budget-party.mjs`, `ui-budget-party.mjs`, `api-budget-reports.mjs`, `ui-budget-reports.mjs` - scratch stack only (`S5_PORT_OFFSET`, `S5_STACK_DIR`); the document, party and report suites need the stack built with `S5_STORAGE=1` (`mkScratchStack.mjs` keeps Storage off otherwise).
