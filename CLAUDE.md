...
## Multi-Entity Owner (Platform Stages 5 + 6 - CLOSED / PASS, live in Production since 2026-09-11)

The fifth independent identity (after the campaign user, the Election Day PermissionUser, the Election Owner and the Platform Owner): the Stage 4 singleton seat, who sees ONLY the workspaces currently assigned to the seat, and only aggregate counts of them. Full record: `CURRENT_STATUS.md`'s Stage 5 and Stage 6 sections.
...
- **Tests** live in `scripts/stage5/` and `scripts/stage6/` and run ONLY against an isolated scratch stack built by `scripts/stage5/mkScratchStack.mjs` (never the developer's `kolbox` stack, never Production). The committed `supabase/config.toml` stays TOTP-disabled on purpose.
- **Stage 7** is authoritatively the **Multi-Entity Owner frontend/dashboard** (not started); Stages 5 (authentication) and 6 (aggregate-only read backend) are closed. Its binding contract is in `CURRENT_STATUS.md`'s Stage 6 section.
