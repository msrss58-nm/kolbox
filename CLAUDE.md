...
## Multi-Entity Owner (Platform Stage 5 - implemented and verified LOCALLY, not yet committed/deployed)
...
- **Own origin / build surface** `VITE_APP_SURFACE=multi_entity` (target Vercel project `kolbox-multi-entity`), never part of `both`. Routes `/multi-entity/login`, `/multi-entity/set-password` (outside the guard), `/multi-entity`. Own client `multiEntityOwnerAuthClient` (key `kb-multi-entity-owner-auth-token`, `detectSessionInUrl: false`).
