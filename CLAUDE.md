...
## Multi-Entity Owner (Platform Stage 5 - CLOSED / PASS, live in Production since 2026-09-11)

The fifth independent identity (after the campaign user, the Election Day PermissionUser, the Election Owner and the Platform Owner): the Stage 4 singleton seat, who sees ONLY the workspaces currently assigned to the seat. Full record: `CURRENT_STATUS.md`'s Stage 5 section.
...
- **Entity scope is DB-authoritative and never cached**: `multi_entity_assert_workspace_assigned(uuid, uuid)` is granted to no role; every future (Stage 6) workspace-scoped read RPC must call it inside its own body, in the same transaction as the read. Responses carry authorization metadata only - never `login_code`, never business data.
...
- **Tests** live in `scripts/stage5/` and run ONLY against an isolated scratch stack built by `mkScratchStack.mjs` (never the developer's `kolbox` stack, never Production). The committed `supabase/config.toml` stays TOTP-disabled on purpose.
- **Stage 7** is authoritatively the **Multi-Entity Owner frontend/dashboard**; Stage 6 is the aggregate-only read backend.
