...
- **`electionDay.manageNonVotingReasons` cannot currently be granted to any role** - shipped, migrated, and deployed (commit `2c92470`), but `election_day_is_valid_permission` (the DB-side role-permission allowlist, defined in the earlier Dynamic Roles Phase 2 migration) was never updated to recognize this string, confirmed live: `election_day_create_role`/`update_role` reject it with `INVALID_PERMISSION`. This blocks only the write path for _saving a role's permission set_ - the non-voting-reasons catalog RPCs themselves are fully live and RPC-verified (see the bullet above), only the "ניהול סיבות אי-הצבעה" screen is unreachable by any real account until a follow-up migration adds this permission string to that allowlist. Not yet written or applied, pending approval - see `CHANGELOG.md`'s matching entry.
