...
- **The `manager`/`operations`/`voting` permission engine (`src/permissions/`) does not change any of the above** - it only decides which buttons/fields render in the UI. Every mutation still goes through the same permissive-RLS PostgREST calls regardless of role; a caller holding the anon key can invoke any of them directly, bypassing every `<PermissionGuard>`. Gating the mutation handlers themselves (not just their UI triggers) and real server-side RPC-based enforcement are separate, not-yet-approved future stages.
