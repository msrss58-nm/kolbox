...
## Budget module ("ניהול תקציב") - Stages 1-5 CLOSED / PASS (Stages 3-5 LOCAL only)

Stages 3 (Budget Core), 4 (Documents & Supplier Workflow) and 5 (Party Funding Workflow) are local commits only - NOT pushed, NOT deployed, their seven migrations (`20260917000000`/`010000`/`020000`, `20260918000000`/`010000`, `20260919000000`/`010000`) NOT applied to Production, Budget not activated anywhere. Stage 6 (Dashboard, Reports & Controls) is NOT STARTED and needs explicit approval. Approved design: `CURRENT_STATUS.md`'s Budget section; the Stage 3, 4 and 5 build records follow it - do not re-derive or reopen them.
...
- **Tests:** `scripts/budget/api-budget.mjs`, `ui-budget.mjs`, `api-voterfile.mjs`, `api-budget-docs.mjs`, `ui-budget-docs.mjs`, `api-budget-party.mjs`, `ui-budget-party.mjs` - scratch stack only (`S5_PORT_OFFSET`, `S5_STACK_DIR`); the document and party suites need the stack built with `S5_STORAGE=1` (`mkScratchStack.mjs` keeps Storage off otherwise).
