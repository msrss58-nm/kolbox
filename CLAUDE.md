...
Added 2026-08-24 after a live Production privilege-escalation incident (see task-plan.md's Progress Log for the full record: `election_day_backfill_historical_workspace` was correctly `REVOKE ALL FROM PUBLIC ... GRANT TO service_role` in the migration itself, but Production carried a project-level `pg_default_acl` entry that auto-granted EXECUTE to `anon`/`authenticated` on every newly-created function anyway - undetected until a direct post-apply `pg_proc.proacl` check on Production itself, since no local disposable replica of this project had that legacy default-privilege configuration at the time). **Corrected 2026-09-11 (Platform Stage 5):** the current local Supabase stacks DO carry an equivalent `pg_default_acl` that auto-grants EXECUTE on new `public` functions to `anon`/`authenticated`/`service_role` (verified by direct catalog read), so local suites now reproduce the hazard - the explicit revokes and the direct Production `pg_proc.proacl` read below remain mandatory regardless.
...
- `src/features/election-day/electionDaySession.ts` - a zustand store (`useElectionDaySession`) that checks an entered name/password against `ApiClient.listPermissionUsers()` (the local roster - originally managed from a worker "ניהול הרשאות משתמשים" modal; since Platform Stage 9 the roster is managed ONLY by the Election Owner, see the user-management bullet below - `PermissionUser { id, name, password, role: "user"|"manager" }`, plaintext, `MockApi`/localStorage only - there is no hashing and no real security boundary here, by design, for an internal tool). A successful login persists just `{id, name, role}` to its own localStorage key so a refresh doesn't sign out.
- **User management is Election Owner authority only (Platform Stage 9 - supersedes both the old empty-roster `isBootstrap` open window in `ElectionDayGuard.tsx` and the Stage 3B mandatory first-PermissionUser bootstrap; neither exists any more).** The Election Owner - never a PermissionUser, exactly one per workspace, never duplicated into the roster - manages users in the Owner administration shell (`/election-day/owner/*` - Users / Roles & Permissions / Modules / Settings, each a real child route of `OwnerAdminShell`, behind `OwnerAuthGuard`; the shared fixed-viewport `AdminShell` in `src/components/admin/` also hosts the Platform console's `/platform/*` sections): list, create and delete Managers and ordinary users, and reset **ordinary (non-Manager) users'** passwords, through `api/election-day/owner-actions.ts` (`list_permission_users` / `create_permission_user` / `delete_permission_user` / `reset_permission_user_password`) with the Owner JWT plus a fresh one-time Owner proof; every RPC re-resolves the Owner's workspace live and treats a target outside it as nonexistent (same-workspace only). A role is a Manager role ONLY through the explicit `election_day_roles.is_manager` flag, which the Owner sets in the role editor (new workspaces seed "מנהל" with `is_manager = true`; later permission changes never change it); the Owner cannot reset a Manager-role user's password (`CANNOT_RESET_MANAGER` - delete and recreate instead). A workspace with zero PermissionUsers is a valid, recoverable state: provisioning ends on the administration page, and reload / sign-out / sign-in always return the Owner there. Workers - Managers included - have no user-management screen, route or API: `api/election-day/permission-users.ts` always answers 403 `USER_MANAGEMENT_OWNER_ONLY`, `reauth.ts` mints no user-management proofs, and the worker `election_day_*_permission_user*_v3` RPCs and `election_day_bootstrap_first_permission_user` have no grant at all. `electionDay.manageUsers` is an inert catalog value (grants nothing, not offered in the role editor). Roles/permissions management stays Owner-only. **Workspace module entitlements** (`platform_modules` catalog + `election_workspace_modules`): the Platform Owner chooses a workspace's modules explicitly at approval (required) and edits them later from the console; every approval choice, provisioning grant, enable, disable and migration backfill is written to the append-only `platform_entitlement_audit` in the same transaction; an approval without a module choice fails closed at provisioning (`APPROVAL_MODULES_MISSING`). Election Day requires the `election_day` entitlement, enforced server-side: worker login answers 403 `MODULE_NOT_ENABLED` (only after the password verified), `election_day_resolve_session` resolves nothing for an unentitled workspace (existing sessions die on their next request), the Owner's Election Day data ops are refused in `owner-actions.ts`, and Multi-Entity aggregates report such a workspace as `unavailable` with no numbers. Owner user/role administration stays available without the entitlement. An entitlement is platform licensing, never a substitute for a role permission.
...
## Multi-Entity Owner (Platform Stages 5 + 6 + 7 - CLOSED / PASS, live in Production since 2026-09-11)

The fifth independent identity (after the campaign user, the Election Day PermissionUser, the Election Owner and the Platform Owner): the Stage 4 singleton seat, who sees ONLY the workspaces currently assigned to the seat, and only aggregate counts of them, on a live dashboard. Full record: `CURRENT_STATUS.md`'s Stage 5, Stage 6 and Stage 7 sections.

- **Own origin / build surface** `VITE_APP_SURFACE=multi_entity` (Vercel project `kolbox-multi-entity`, `https://kolbox-multi-entity.vercel.app`), never part of `both`. Routes `/multi-entity/login`, `/multi-entity/set-password` (outside the guard), `/multi-entity`. Own client `multiEntityOwnerAuthClient` (key `kb-multi-entity-owner-auth-token`, `detectSessionInUrl: false`).
- **Server boundary**: `api/platform/_multiEntityAuth.ts` - `getUser` FIRST, then `getClaims` (`aal === "aal2"`, `sub` match), then `multi_entity_resolve_owner_context` - is the ONLY Multi-Entity authentication primitive. Never unify it with `verifyOwnerJwt` or `verifyPlatformOwnerJwt`, and never authorize by email/metadata/URL/client state. Reached only through the `me_op` partition at the top of `api/platform/session.ts` (12/12 functions - no new file possible); `op` + `me_op` is refused.
- **Entity scope is DB-authoritative and never cached**: `multi_entity_assert_workspace_assigned(uuid, uuid)` is granted to no role; every workspace-scoped read RPC must call it inside its own body, in the same transaction as the read (Stage 6's internal `multi_entity_compute_workspace_aggregate` does so as its first statement). The Stage 5 ops carry authorization metadata only - never `login_code`, never business data.
- **Aggregates (Stage 6)**: `GET /api/multi-entity/aggregates` (every current assignment + totals) and `GET /api/multi-entity/workspace-aggregates?workspaceId=` (`me_op=aggregates|workspace_aggregates`). Counts only - `contactsTotal`, `voted`, `followUpClosed`, `followUpRemaining`, `rideNeeded`, `rideArranged`, `rideCompleted` - mirroring the Election Day dashboard definitions (`followUpStatus.ts`, `rideStatusBreakdown.ts`). **The privacy rules live in the DB**: an ended workspace (`election_end_at <= now()`) is `ended` and one with fewer than 10 contacts is `suppressed`, both with `metrics: null` (never zeros); totals are summed from `reported` rows only. Never add a coordinator, per-reason, time-series or person-level metric without a new privacy review.
- **Exclusive principal**: the resolver accepts only `multi_entity_auth_user_held_by(id) = 'multi_entity'`; a seat holder who also holds a Platform/Election Owner role is refused. The reverse direction (that identity keeping its other role) is the documented D-8 residual.
- **No self-service recovery.** Lost password/device -> the Platform Owner replaces the seat (same email: replace with interim B -> purge A -> replace back to A -> purge B).
- **Dashboard (Stage 7)**: `/multi-entity` (server `totals` as returned + one card per assigned workspace, server order) and `/multi-entity/workspaces/:workspaceId` (single-workspace endpoint; unassigned/nonexistent -> "not available"). Data comes ONLY from the Stage 6 aggregate endpoints via `useMultiEntityAggregates.ts` (`useAsyncData`, keyed on the session store's `context`, so every authoritative re-resolution - entry, tab visible, "רענון" - triggers a fresh read; no polling). Rules: render numbers only for `reported`; `metrics: null` is never 0; no summary tiles when nothing reports; never persist aggregates in any browser storage; `multiEntityOwnerClient.ts` rejects any response that is not exactly the Stage 6 shape. The Stage 5 UI suite relies on the heading "המערכות שלי", the empty-state text, `[data-testid="workspace-list"] li` and the "רענון" button - keep them.
...
## Stage 8D security remediation (CLOSED / PASS, live in Production since 2026-09-12)

Closes the three HIGH findings the Stage 8C adversarial lab reproduced (migration `20260915000000` + `api/platform/session.ts`; Production 90/90 zero drift; three surfaces on `a77ae8c`). Scope was exactly these three.

- **H-1 (anon cross-workspace reason management)** and **H-2 (anon cross-workspace deadline write)** - see the "Known Security Limitations" bullets above (both now fixed and marked). H-2: the vestigial `election_day_settings` singleton lost its `anon`/`authenticated` table privileges and its permissive `election_day_settings_all` policy; RLS stays enabled and the DEFINER reverse-sync trigger still mirrors trusted writes into the singleton (rollback safety) because it runs in the definer chain as postgres. Nothing was dropped. Verified in Production: policy gone, anon INSERT/UPDATE/SELECT all false, reverse-sync triggers present; live anon-key UPDATE/INSERT → 42501.
- **H-3 (re-issued Owner recovery link survived provisioning → live Owner takeover)**: `election_day_provision_workspace` now invalidates the account's outstanding recovery token - `delete auth.one_time_tokens where token_type='recovery_token'` + clear `auth.users.recovery_token` (both empirically proven on the pinned GoTrue to invalidate an outstanding link) - IN THE SAME TRANSACTION that consumes the approval. The reissue-after-consume race is closed by `platform_reissue_finalize`, which the reissue handler calls AFTER minting: if a racing provisioning consumed the approval during the mint, it invalidates the just-minted token and the handler returns 409. Both new functions are DEFINER/`search_path=''`/service_role-only. Legitimate pre-provision recovery and expired-approval renewal are unchanged. Proof is local (no real Production Owner created): retained `scripts/stage8d/api-stage8d.mjs` exploit-blocked + full race matrix (provision-first 409, finalize race, simultaneous, two-reissue, response-lost).
...
## Budget module ("ניהול תקציב") - Stages 1-6 + 7A CLOSED / PASS; Stage 7B Gate 2 (Production migrations) CLOSED / PASS

Stages 3 (Budget Core), 4 (Documents & Supplier Workflow), 5 (Party Funding Workflow), 6 (Dashboard, Reports & Controls) and 7A (Integration, Security & Release Readiness) are local commits only - NOT pushed, NOT deployed (all three surfaces still serve `7d3f90b`). Their nine migrations (`20260917000000`/`010000`/`020000`, `20260918000000`/`010000`, `20260919000000`/`010000`, `20260920000000`, `20260921000000`) ARE applied to Production since 2026-09-15 (Stage 7B Gate 2: 101/101, zero drift, business data byte-identical) with Budget inert - `platform_modules.budget.available = false`, zero `budget` entitlements, zero Budget rows. Stage 7B rollout order (corrected - DB first, because a push to `master` auto-deploys all three surfaces and the Budget code depends on the Budget schema): Gate 2 migrations CLOSED / PASS -> Gate 1 push / auto-deploy NEXT -> Gate 3 deploy verification -> Gate 4 controlled activation -> Gate 5 acceptance -> Gate 6 closure; each gate needs its own explicit approval - the gate plan and the acceptance checklist are in `CURRENT_STATUS.md`'s Stage 7A section, the Gate 2 record in its Stage 7B Gate 2 section. Approved design: `CURRENT_STATUS.md`'s Budget section; the Stage 3-7A build records follow it - do not re-derive or reopen them.
...
- **Documents (Stage 4):** private Storage bucket `budget-documents` (10 MiB; PDF/JPEG/PNG/HEIC/HEIF only; a RESTRICTIVE `storage.objects` policy denies anon + authenticated) at server-generated paths `<workspace_id>/<uuid>` - the user's file name is sanitized display metadata, never part of the path. Upload = `document_upload_start` (DB intent, 15 min) -> signed upload URL -> the browser PUTs straight to Storage -> `document_upload_complete` (the handler downloads the object and verifies size, magic bytes AND the stored Content-Type against the declared type, computes sha256, then finalizes - or deletes the object and rejects). Download = a fresh 60-second signed link per click (attachment). `budget_document_versions` are append-only (a replacement is the next version; archive is the only delete-like action; documents are never deleted). The DB ops that take a path / hash / snapshot (`document_upload_lookup|finalize|reject`, `document_version_locate`, `order_form_data|record`) are handler-internal and refused from clients.
- **ONE requirement engine:** `budget_document_requirements_live(ws, expense)` decides required / conditional / present / missing / satisfying version, invoice + supplier-signature + photo required, and readiness; `budget_expense_requirements` shows the close-time snapshot for a closed expense (`budget_expense_requirement_snapshots`, append-only, a new one per close). Thresholds are rule DATA (`amount_gt` = strictly greater; default 150000 agorot = 1,500 ILS, exactly 1,500 is not required). Photo = category rule or the per-expense flag (`budget_expense_document_flags`, only for a type with an active `manual` rule). The close guard adds `REQUIRED_DOCUMENTS_MISSING` / `ORDER_FORM_INCOMPLETE` from this engine. Never re-implement rule logic in the UI.
- **Order form PDF:** server-side only, pdf-lib + the embedded OFL David Libre font (`api/budget/_orderFormFont.ts`, licence `api/budget/_fonts/`), layout/wording in the versioned template `_orderFormTemplate.ts` (`kolbox-order-form-v1`), renderer `_orderFormPdf.ts` (own bidi pass; fontkit is forced to lay out LTR because it would otherwise reverse whole Hebrew strings, digits included). Deterministic (fixed font names, no dates). Preview is watermarked and never stored; the FINAL form requires the prior budget approval on every party allocation; regeneration is version N+1 (N kept); a supplier-signed return is linked to its generated version and satisfies the signature requirement only for the LATEST version; "sent" is only the user's confirmation. The approval decision itself stays in the party workflow (Stage 5).
- **Owner step-up sign-out must be `scope: "local"`:** the isolated `signInWithPassword` session is revoked with `signOut({ scope: "local" })`, awaited - the supabase-js default (`global`) revokes every Owner session, signing the Owner out after one step-up (found and fixed in Stage 4).
- **Party funding (Stage 5):** keyed by the PARTY ALLOCATION, as four separate business events - prior approval (`budget_party_preapprovals`), submission (`budget_party_submissions` + the append-only `budget_party_submission_events`: `ready`/`sent`/`returned`, attempt number, sent form version, requested amount, package, optional idempotency key), payment reference, and party payments (supplier-ledger rows, payer `party` - the party pays the supplier directly; never campaign cash). The process state comes from ONE function (`budget_party_workflow_state`, exposed per allocation by `budget_party_facts` - the Stage 6 source) and the gates from ONE function (`budget_party_submission_blockers`, whose document part IS the Stage 4 engine); payment status is a separate axis. `mark_submission_ready` / `mark_submission_sent` re-check every gate server-side; a party allocation can never be raised above the pre-approved amount; nothing ever auto-fills a funding gap. Never add a second derivation of state, gates or balances in the UI or in Stage 6.
- **Dashboard & reports (Stage 6):** `get_dashboard` (budget.view) and seven `report_*` ops (budget.viewReports) read ONLY the calculation layer: the Stage 3 facts for money, `budget_party_facts` for the party process, and the requirement engine in its set-based form `budget_requirement_items(ws, ids | null)` (Stage 4's `budget_document_requirements_live` is a wrapper over it - one engine). Queue definitions live only in `budget_expense_queue_facts` and list / report filters only in `budget_expense_filter`, so every dashboard count equals the list it drills into - never add a queue or filter predicate anywhere else. KPIs: total budget = active sources; total expenses = committed + actual; available = total budget - committed - actual. Report lists are server-paged (limit <= 200) with unique tie-breakers; totals cover the whole filtered set. Bulk read helpers are plpgsql with `plan_cache_mode = force_custom_plan` (a `LANGUAGE sql` set function is planned with opaque parameters - ~15x slower). No bank-account data in any report. Report export is NOT built (outside the approved scope).
- **Workspace delete guard + deletion export (Stage 7A):** a workspace holding ANY Budget row can be deleted permanently only with a fresh, VERIFIED deletion export - the BEFORE DELETE trigger `election_workspaces_budget_delete_guard` compares the current content fingerprint (`budget_export_fingerprint`: every Budget data table incl. the append-only audit, the access log excluded) with the export's, atomically with the delete, then purges the Budget rows in dependency order and writes the append-only `budget_workspace_deletions` (it outlives the workspace); otherwise `BUDGET_EXPORT_REQUIRED` / `BUDGET_EXPORT_STALE`. There is still no product path that deletes a workspace. The export is Election Owner only (`export_status` / `export_start` / `export_part` / `export_document` / `export_verify`, permission `'{owner}'` + `budget_require_owner`): 200-row parts, sha256 of each part's exact JSON text, documents one at a time through 60-second signed links, verify = every part and document served by THAT export and client-confirmed + content unchanged. Bank account numbers are exported masked. A new Budget table MUST be added to `budget_export_tables()` (or be export / deletion / cleanup bookkeeping) and to the purge order - the Stage 7A suite fails otherwise. The immutability triggers' only exception is the transaction-local `kolbox.budget_purge = <workspace>` set inside `budget_purge_workspace_data` (granted to no role) - never set it anywhere else.
- **Download audit + Storage cleanup (Stage 7A):** every authorized signed-link issuance writes one row to the append-only `budget_document_access_log` (reads - not part of the export fingerprint). Orphaned objects (no document version, no live upload intent, older than 1 h - incl. a purged workspace's files) are listed by `budget_storage_orphans` and removed in batches of 200 through the Storage API by `GET /api/budget/actions` with `Authorization: Bearer <CRON_SECRET>` (no secret = closed, 405); scheduling it (cron + `CRON_SECRET`) is a Stage 7B decision. Supplier lists are set-based (`budget_supplier_list_json`) - never call a whole-workspace facts function per row.
- **Tests:** `scripts/budget/api-budget.mjs`, `ui-budget.mjs`, `api-voterfile.mjs`, `api-budget-docs.mjs`, `ui-budget-docs.mjs`, `api-budget-party.mjs`, `ui-budget-party.mjs`, `api-budget-reports.mjs`, `ui-budget-reports.mjs`, `api-budget-stage7.mjs`, `ui-budget-stage7.mjs` - scratch stack only (`S5_PORT_OFFSET`, `S5_STACK_DIR`); the document, party, report and Stage 7 suites need the stack built with `S5_STORAGE=1` (`mkScratchStack.mjs` keeps Storage off otherwise).
...
- **`election_day_login` (legacy) is retired** - Multi-Tenant Phase 4A (migration `20260830010000`) revoked its `anon`/`authenticated` EXECUTE grant; it was already dead in the app (the live session flow uses `election_day_login_v2` exclusively via `POST /api/election-day/session`, confirmed with zero live callers before retirement). The live path, `election_day_login_v2`, **does** have rate limiting: `api/election-day/session.ts` calls `election_day_register_login_attempt` (a standalone, separately-committed Postgres call) before ever calling `election_day_login_v2` itself - see Phase 3A's Session/Reauth v3 record in `CURRENT_STATUS.md` for why the attempt-count has to be a separate call rather than counted inside the login RPC itself.
- **`election_day_list_permission_users()` is retired** - migration `20260828010000_election_day_retire_legacy_list_permission_users.sql` revoked its `anon`/`authenticated` EXECUTE grant once the frontend roster read was cut over to the trusted, session-scoped `election_day_list_permission_users_v3` (via `GET /api/election-day/permission-users`). No caller holding only the anon key can invoke it anymore. _(Platform Stage 9: `election_day_list_permission_users_v3` has since lost its only grant too, and `GET /api/election-day/permission-users` always answers 403 `USER_MANAGEMENT_OWNER_ONLY` - the roster is read only by the Election Owner via `owner-actions?op=list_permission_users`.)_
...
- **Three Vercel projects now build this same repo/branch (`master`), distinguished only by `VITE_APP_SURFACE`**: `kolbox` (`election`, `kolbox-gamma.vercel.app`), `kolbox-platform` (`platform`, `kolbox-platform.vercel.app`) and `kolbox-multi-entity` (`multi_entity`, `kolbox-multi-entity.vercel.app`, since 2026-09-11). All three auto-deploy on push; each `/api/health` reports `{commit, surface}`. The Platform project carries `KOLBOX_MULTI_ENTITY_APP_BASE_URL=https://kolbox-multi-entity.vercel.app` (without it, Multi-Entity provisioning fails closed in production). To deploy an exact commit without a push, use the Vercel Create Deployment API with `gitSource` pinned to the SHA - it keeps Git metadata, whereas a raw `vercel --prod` upload may not.
