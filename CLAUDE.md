...
## Budget module ("ניהול תקציב") - Stages 1-4 CLOSED / PASS (Stages 3 + 4 LOCAL only)

Stages 3 (Budget Core) and 4 (Documents & Supplier Workflow) are local commits only - NOT pushed, NOT deployed, their five migrations (`20260917000000`/`010000`/`020000`, `20260918000000`/`010000`) NOT applied to Production, Budget not activated anywhere. Stage 5 (Party Funding Workflow) is NOT STARTED and needs explicit approval. Approved design: `CURRENT_STATUS.md`'s Budget section; the Stage 3 and Stage 4 build records follow it - do not re-derive or reopen them.
...
- **Tests:** `scripts/budget/api-budget.mjs`, `ui-budget.mjs`, `api-voterfile.mjs`, `api-budget-docs.mjs`, `ui-budget-docs.mjs` - scratch stack only (`S5_PORT_OFFSET`, `S5_STACK_DIR`); the two document suites need the stack built with `S5_STORAGE=1` (`mkScratchStack.mjs` keeps Storage off otherwise).
