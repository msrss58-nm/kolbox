/** Shared fixtures for the Dynamic Roles & Permissions Node smoke tests.
 * `BUILT_IN_ROLE_SEED` is re-exported (not re-typed) from the shipped app
 * code so tests exercise the exact same data the real `MockApi` returns -
 * see `scripts/smoke-role-seed-parity.ts` for the check that this array (via
 * the shipped module) never drifts from the Phase 0 migration's SQL seed.
 * `MALFORMED_ROLE_ROWS` are hand-built untrusted rows for exercising
