...
/** Navigation Refactor: extracted from the old `RideCoordinatorsModal` - same
 * content, no `Modal` wrapper, now rendered as a tab on `/election-day/rides`
 * instead of a dialog. */
export function RideCoordinatorsPanel({
  coordinators,
  onAdd,
  onDelete,
}: {
  ...
  return (
    <div className="space-y-5">
      ...
      {coordinators.length === 0 ? (
        <EmptyState icon={UserPlus} title={ELECTION_DAY_TEXT.coordinatorsManager.empty} />
      ) : (
