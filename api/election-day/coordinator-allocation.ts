...
// Phase 3C Coordinator/Allocation Dual-Principal V3 - PermissionUser session
// path. GET -> workspace-scoped coordinator roster (election_day_
// list_coordinators_v3). POST -> manage_coordinators/apply_initial_
// allocation/rebalance_assignments/end_coordinator_activity, selected by
// body.op. Browser -> __Host-kb_ed_session HttpOnly cookie [+, for POST, a
// previously-issued action-bound proof from POST /api/election-day/reauth]
// -> this function hashes the session cookie (and, for POST, the proof) in
// Node -> calls the matching election_day_<op>_v3 RPC (20260829030000).
// Actor/role/workspace are derived entirely server-side inside those RPCs
// from the session hash - this endpoint never accepts or forwards a
// client-supplied actorId/workspaceId/roleId.
//
// Mirrors api/election-day/roles.ts's own shape (one file for the read plus
// all mutations, GET has no Origin check, POST does, fail-closed body-key
// allowlist, generic error-code mapper) and api/election-day/reauth.ts's own
// session-cookie/hashing conventions exactly.
//
// NOT wired into the live frontend by this migration/EXPAND - useCoordinator
// Allocation.ts and every existing UI component (CoordinatorRosterEditor,
// CoordinatorAllocationSetup, RebalanceDialog, EndCoordinatorDialog)
// continue to call the legacy election_day_*_v2 RPCs directly, unchanged.
// This endpoint exists ahead of that separate, later, explicit frontend
// cutover decision.
...
// (matching reauth.ts's ALLOWED_REAUTH_ACTIONS allowlist) when it calls
// election_day_verify_and_consume_reauth_proof_v3 - this endpoint never
// passes an action itself, only routes to the correct RPC by op.
const OP_TO_RPC: Record<string, string> = {
  manage_coordinators: "election_day_manage_coordinators_v3",
  apply_initial_allocation: "election_day_apply_initial_allocation_v3",
  rebalance_assignments: "election_day_rebalance_assignments_v3",
  end_coordinator_activity: "election_day_end_coordinator_activity_v3",
};
...
export default async function handler(
  ...
  // op-specific required fields. Client sends only business payload +
  // proof - it never selects a workspace or a trusted actor identity; those
  // are derived entirely server-side inside the RPC from the session hash.
