...
// "clear_voters") -> this function hashes both in Node -> calls
// election_day_clear_voters_v3 (20260829050000). Actor/role/workspace are
...
// roleId. Mirrors api/election-day/import-voters.ts's own shape exactly.
//
// NOT wired into the live frontend by this EXPAND - the legacy frontend
// Clear path (SupabaseElectionDayApi.clearElectionDayVoters, a raw
// unguarded PostgREST delete with zero reauth and zero workspace scope)
// continues unchanged. This endpoint exists ahead of that separate, later,
// explicit frontend cutover decision.

const ALLOWED_BODY_KEYS = new Set<string>(["reauthProof"]);
