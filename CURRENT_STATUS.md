...
## Coordinator Sync + Voter Filter Fix - status (2026-08-20) - MIGRATION APPLIED TO PRODUCTION; FRONTEND STILL LOCAL-ONLY, NOT PUSHED, NOT DEPLOYED

**Split state - read carefully, the DB half and the frontend half are at different stages.** The migration below is live in Production, verified read-only. The frontend commit (`685d729`, "fix: sync election coordinators and repair voter filters") that carries both the coordinator-sync UI and the Issue 2 voter-filter fix is still local-only - not pushed to `origin/master`, not deployed to Vercel. **Do not read this section as saying the Issue 2 fix or the inferred-coordinator "אחראים שזוהו בנתוני הבוחרים" UI are live for real users yet - they are not**, since Production is still serving whatever frontend bundle was last deployed before this work.

**Committed files** (commit `685d729`, local-only): `src/features/election-day/CoordinatorRosterEditor.tsx`, `coordinatorAllocationStats.ts`, `election-day.constants.ts`, `useElectionDay.ts`, and the migration `supabase/migrations/20260820000000_election_day_import_coordinator_sync.sql`.
...
**Remaining rollout steps (not yet done)**: `git push` the local `685d729` frontend/migration-record commit plus the documentation-sync commit that recorded this Production apply → confirm Vercel auto-deploy → bundle-hash-verify the live frontend actually serves the Issue 2 fix and the inferred-coordinator UI, mirroring this project's standard rollout pattern for every prior Election Day change. The DB side of this work is done; the frontend side is not live until these steps happen.
