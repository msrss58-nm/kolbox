...
## Coordinator Sync + Voter Filter Fix - status (2026-08-20, LOCAL ONLY - not pushed, not deployed, migration not applied)

**Do not read this section as describing live Production behavior.** Everything below is implemented and verified only on this machine / a local disposable Supabase instance (Docker). No commit exists for it yet as of this note being written (it lands in the same commit as this documentation update, per the approved scope for that pass - check `git log` for the actual current state before relying on this).

**Pending files** (all local, uncommitted as of the start of this note): `src/features/election-day/CoordinatorRosterEditor.tsx`, `coordinatorAllocationStats.ts`, `election-day.constants.ts`, `useElectionDay.ts`, and a new migration `supabase/migrations/20260820000000_election_day_import_coordinator_sync.sql`.
...
**Verified locally**: 3 full `npx supabase db reset` replays (Docker, all 42 migrations) with zero errors; the bidirectional concurrency test above; a forced NOT-NULL-violation test confirming voter import and coordinator sync roll back together atomically; permission/reauth regression checks unchanged (`FORBIDDEN` still correctly raised, zero mutation on every failure path); `tsc -b`/`npm run build`/`eslint`/`git diff --check` all clean. **Not performed**: live browser/Production click-through, migration apply, push, or deploy.

**Production read-only baseline recorded** (nothing written): **1,420 voters**, **0 persisted coordinators**, **5 distinct missing coordinator names** - חיה, נועם, קארן, לוסיאנה, עדי. This confirms the previously-documented "1,928 voters" figure elsewhere in this file is stale for the voter-count purpose (kept below only as historical record of when it was true; the 1,420 figure above is current as of this section's own read).

**Exact next step**: review/commit approval → commit → (separately, explicitly approved) `supabase db push` to apply the migration to the linked remote project → re-verify the 5 names landed and nothing else changed → push to `origin/master` → confirm Vercel auto-deploy → bundle-hash-verify the live frontend, mirroring this project's standard rollout pattern for every prior Election Day change. None of these steps have happened yet.
