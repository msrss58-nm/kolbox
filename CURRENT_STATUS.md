...
- **URL**: `https://kolbox-gamma.vercel.app` - live since 2026-07-19. **Active and serving Dynamic Roles & Permissions Phase 3 - the full 4-phase rollout is now closed.**
- **Vercel project**: `kolbox`, scope `nahom10`, Git integration auto-deploys on push to `master`.
- Current production deployment: `dpl_DvJscBWx42YSKQRQHdyDEbKwmP5p`, `readyState: READY`, `target: production`, built from commit `2e8191c` ("feat: remove legacy role model (Phase 3)") - created ~24 seconds after that commit's push, no other push in between. Aliased to `kolbox-gamma.vercel.app` / `kolbox-nahom10.vercel.app` / `kolbox-git-master-nahom10.vercel.app`.
- Verified live on 2026-08-06 (post-deploy production smoke test, full Playwright run against the `2e8191c` deploy): HTTP 200; **20/20 checks passed** - creating a new `PermissionUser` via the renamed RPC works again (closing the transient gap disclosed after the migration was applied but before this deploy); manager/operations/a dynamic-role test account all logged in via the real UI (temporary RPC-created accounts, deleted immediately after); manager saw the full 1,928-voter dataset and every admin control; created/edited/cloned/deleted a custom role and created a `PermissionUser` against it; the delete button for a role with an assigned user was correctly disabled; operations/the dynamic-role account saw none of the admin-only buttons they weren't granted; logout/login round-tripped cleanly; 0 console/page errors. Real data confirmed unchanged after cleanup: **1,928 voters**, `PermissionUser` roster count returned exactly to baseline (0).

## Git

- Branch: `master`
- HEAD: `2e8191c` ("feat: remove legacy role model (Phase 3)")
